...
def build_change_map(articles):
    ...
    for art in articles:
        ...
        if aid not in stored_hashes or stored_hashes[aid] != h:
            to_process.append(aid)  # new or updated
        else:
            skip.append(aid)  # unchanged
...
# ---------------- Step 3: AI Process (Clean & Structure) ----------------
...
structured_output = [ai_clean_and_structure(a["body"]) for a in to_process_articles]
...
print(f"\nStored {len(id_map)} articles in FAISS index.")
...
# Simulate feedback
update_index_with_feedback(query, "A12345", "helpful")
refresh_articles()